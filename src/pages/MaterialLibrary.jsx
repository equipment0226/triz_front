...
export function MaterialLibrary({ id, go, chapter, standard, principle }) {
  ...
  const filtered = indexed.filter(e => (category === 'all' || category === e.category) && (functionFilter === 'all' || e.subtitle === functionFilter) && e.searchText.includes(query.trim().toLocaleLowerCase()));
  const shown = id === 'effects' ? filtered.slice(0,visible) : filtered;
  ...
  return <><GuideLink className="guide-back" go={go} to={chapter ? {chapter:chapter.id} : {}}><ArrowLeft size={16}/>{chapter ? `${chapter.id} ${chapter.title}` : 'Tool 소개로'}</GuideLink><section className="library-hero"><div><p className="eyebrow">THE REFERENCE SHELF / {meta.en}</p><h1>{meta.title}</h1><p className="guide-lead">{meta.description}</p><span className="library-total"><BookOpen size={15}/>{totalLabel}</span></div>{id === 'business' ? <figure className="business-hero-photo"><img src="/images/business-collaboration.jpg" alt="데이터를 함께 검토하며 사업 문제를 논의하는 팀" width="1000" height="1500"/><figcaption>사진: <SafeLink href="https://www.pexels.com/photo/a-group-of-people-having-a-meeting-in-the-office-7108816/">Tiger Lily / Pexels</SafeLink></figcaption></figure> : <GuideVisual kind={meta.visual} title={`${meta.title} 개념도`}/>}</section>
    ...
    <div className="library-toolbar"><label className="library-search"><Search size={18}/><input type="search" aria-label="자료 검색" placeholder="번호, 이름, 원리 또는 조건으로 검색" value={query} onChange={e => {setQuery(e.target.value);setVisible(40);}}/></label>{categories.length > 0 && <label className="library-filter"><span className="sr-only">자료 분류</span><select aria-label="자료 분류" value={category} onChange={e => {setCategory(e.target.value);setVisible(40);}}><option value="all">모든 분류</option>{categories.map(([k,v]) => <option key={k} value={k}>{id === 'standards' ? `${k}. ` : ''}{v}</option>)}</select></label>}{id === 'effects' && <label className="library-filter"><span className="sr-only">요구 기능</span><select aria-label="요구 기능" value={functionFilter} onChange={e => {setFunctionFilter(e.target.value);setVisible(40);}}><option value="all">모든 기능</option>{data.effects.map(g => <option key={g.function_ko} value={g.function_ko}>{g.function_ko}</option>)}</select></label>}<span className="library-count" aria-live="polite">{filtered.length} / {entries.length}</span></div>
    {!filtered.length && <div className="guide-empty"><Search size={28}/><h2>일치하는 자료가 없습니다.</h2><p>검색어 또는 분류를 바꿔보세요.</p><button className="button subtle" onClick={() => {setQuery('');setCategory('all');setFunctionFilter('all');setVisible(40);}}>검색 초기화</button></div>}
    {id === 'standards' ? <StandardsExplorer collection={data.standards_76} entries={filtered} selectedCode={standard} go={go} chapter={chapter} searchActive={Boolean(query.trim()) || category !== 'all'}/> : <div className={`library-entries library-${id}`}>{shown.map(entry => {const e=entry.original; return <details className="library-entry" id={`material-${id}-${entry.key}`} key={entry.key}><summary><span className="entry-number">{entry.key.padStart(2,'0')}</span><div>{entry.subtitle && <small>{entry.subtitle}</small>}<h2>{entry.title}</h2><p>{entry.description}</p></div><ChevronDown size={18} className="entry-chevron"/></summary><div className="entry-body">
      ...
      {id === 'separation' && <><GuideVisual kind="separation" title="시간·공간·조건·시스템 수준의 네 가지 분리 축"/><EntryTable label="분리원리 적용 템플릿" rows={[["핵심 질문",e.question],['적용 검토','양쪽 요구가 각각 성립할 구간과 경계를 정의합니다.'],['연계 원리',e.principles.map(p => `#${p} ${data.principles_40[p]?.name_ko}`)]]}/><PrincipleLinks ids={e.principles} go={go} chapter={chapter}/></>}
      {id === 'effects' && <EntryTable label="과학효과" rows={[["자료 식별자",entry.key],["요구 기능",entry.subtitle],["작동 원리",e.principle],["필요 조건",e.conditions]]}/>}
      ...
      {id === 'ariz' && <><GuideVisual kind="ariz" title="문제 모델에서 모순·자원을 거쳐 검증으로 이어지는 ARIZ"/><div className="guide-table-scroll" tabIndex="0" role="region" aria-label={`${entry.title} 단계표`}><table><caption>Part {entry.key} · {entry.title}</caption><thead><tr><th scope="col">단계</th><th scope="col">분석 내용</th><th scope="col">구분</th></tr></thead><tbody>{e.steps.map(step => <tr key={step.code}><td>{step.code}</td><td>{step.title}</td><td><span className={`stage-pill ariz-status ${step.required?'ariz-required':'ariz-optional'}`}>{step.required?'필수':'선택'}</span></td></tr>)}</tbody></table></div></>}
    ...
    </div></details>;})}</div>}{shown.length < filtered.length && <div className="library-more"><p>{shown.length} / {filtered.length}개 표시</p><button className="button subtle" onClick={() => setVisible(v => v+40)}>과학효과 40개 더 보기 <ChevronDown size={16}/></button></div>}<div className="library-footer"><BookOpen size={16}/><span>TRIZ Studio 지식 자료 · 설명을 내 문제의 구체적인 요소와 조건에 연결해보세요.</span></div></>;
}
